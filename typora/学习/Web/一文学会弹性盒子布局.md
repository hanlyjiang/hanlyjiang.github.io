...
零零碎碎的接触弹性布局，但是一直都没有完全掌握，痛定思痛，决定花费些精力彻底掌握。网络上实际上有很多文章，但是感觉都不是我的菜，还是自行总结的更加符合自己的思维习惯。
...
* `flex-direction`： 定义弹性容器的==主轴（需要解释）==
* `flex-wrap`： 是否允许弹性元素换行
* `flex-flow`： `flex-direction`和 `flex-wrap` 的简写
* `justify-content`: 容器主轴上如何分布各行里的弹性元素（如何把弹性容器的空间分配到弹性元素的四周或者弹性元素之间）
* `align-items`: 弹性元素在交叉轴方向的对齐方式（对应的有一个应用于弹性元素的修改单个元素的对齐方式的属性-`align-self`）
* `aligin-content`: 交叉轴方向上的额外空间如何分配到弹性元素之间和周围（针对有多行的弹性容器）
